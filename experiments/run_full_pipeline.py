...
注意：本脚本是 01/02/04 对照轨入口。若要产出 05 主协议要求的 split_manifest/configs_snapshot，
请使用 experiments/05_program/src/runners/run_all.py。
"""
...
def main() -> int:
    ...
    manifest = {
        ...
        "main_protocol_note": "For frozen split_manifest/config snapshots, run experiments/05_program/src/runners/run_all.py",
    }

    print(
        "\n[提示] 当前脚本运行的是 01/02/04 对照轨。"
        "若用于主结论复现，请额外运行 05_program 的 run_all.py 生成 split_manifest 与配置快照。",
        flush=True,
    )
