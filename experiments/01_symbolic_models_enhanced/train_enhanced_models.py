...
from csv_data import (
    CSV_DATA_PATH,
    TEST_SIZE as CSV_TEST_SIZE,
    SPLIT_RANDOM_STATE as CSV_SPLIT_RANDOM_STATE,
    load_csv_standardized_and_targets,
)
...
class EnhancedModelTrainer:
    ...
    def __init__(self, standardizer: EnhancedChoices13kStandardizer,
        ...
        Args:
            ...
            data_source: "json" 使用 c13k 权威 JSON；"csv" 使用 experiments/data/data.csv（与 02 同源）
            csv_path: data_source=csv 时的 CSV 路径，默认 CSV_DATA_PATH
        """
        ...
        # 定义要测试的划分（CSV 模式仅 train_test，与 02 单一划分一致）
        if data_source == "csv":
            self.split_types = ['train_test']
        else:
    ...
    def load_and_prepare_data(self) -> Tuple[List[Dict], np.ndarray]:
        """加载和准备数据。json：权威 c13k JSON；csv：与 02 相同的 data.csv。"""
        if self.data_source == "csv":
            print(f"从 CSV 加载（与 experiments/02 同源）: {self.csv_path}")
            self.standardized_data, y = load_csv_standardized_and_targets(self.csv_path)
            print(f"  记录数: {len(self.standardized_data)}")
            print(f"  目标为 bRate，形状: {y.shape}")
            return self.standardized_data, y

        canonical_json = Path(__file__).resolve().parent / "c13k_enhanced_standardized.json"
        if self.standardized_data is None:
            if canonical_json.exists():
                print("加载标准化数据（与 02 共用权威 JSON）...")
                with open(canonical_json, "r", encoding="utf-8") as f:
                    self.standardized_data = json.load(f)
                self.standardizer.standardized_data = self.standardized_data
            else:
                print("加载和标准化数据...")
    ...
    def evaluate_model(self, y_true: np.ndarray, y_pred: np.ndarray) -> Dict:
        """
        评估模型性能。与 02 train.evaluate_model 指标与公式一致，便于跨实验可比。
    ...
    def _generate_mean_curves(self, timestamp: str) -> None:
        """遍历 curves_dir 下本 run 的 JSON；mean_only 时仅生成四模型对比图，all 时另生成每模型均值图。"""
        try:
            import matplotlib.pyplot as plt
        except ImportError:
            return
        ...
        # 按 split_type 汇总四模型：一张图四条线 (Iteration vs Train MSE)
        by_split: Dict[str, Dict[str, List[Dict]]] = {}
        for (st, mn), group in by_key.items():
            by_split.setdefault(st, {})[mn] = group
        model_order = ["ev", "eu", "pt3", "pt5"]
        colors = {"ev": "#adc178", "eu": "#4f94cd", "pt3": "#e07a5f", "pt5": "#3d405b"}
        for split_type, model_groups in by_split.items():
            max_iter = 0
            for group in model_groups.values():
                for d in group:
                    max_iter = max(max_iter, len(d.get("history") or []))
            if max_iter == 0:
                continue
                ...
                for i, d in enumerate(group):
                    ...
                    for j, h in enumerate(hist):
                        if j < max_iter:
                            train_arr[i, j] = h["train_loss"]
                mean_ = np.nanmean(train_arr, axis=0)
                series[model_name] = mean_
            if not series:
                continue
            fig, ax = plt.subplots(1, 1, figsize=(10, 5))
            iters = np.arange(1, max_iter + 1, dtype=int)
            for model_name, mean_ in series.items():
                label = model_name.upper()
                ax.plot(iters, mean_, label=label, color=colors.get(model_name, "C0"), linewidth=6, alpha=0.8)
            ax.set_xlabel("Iteration", fontsize=17, fontweight="bold")
            ax.set_ylabel("Train MSE", fontsize=17, fontweight="bold")
            ...
            fig.tight_layout()
            out_path = self.curves_dir / f"iteration_vs_train_mse_comparison_{timestamp}_{split_type}.png"
            fig.savefig(out_path, dpi=150)
            plt.close(fig)
            print(f"  符号四模型对比图已保存: {out_path}")
    ...
    def train_model_on_split(self, model_name: str, split_type: str, 
        ...
        # 创建数据划分（train_test 与 02 一致：random_state = SPLIT_RANDOM_STATE + seed）
        split_kwargs: Dict[str, Any] = {}
        if split_type == "problem":
            split_kwargs["test_size"] = 0.2
            split_kwargs["random_state"] = random_state
        elif split_type == "train_test":
            split_kwargs["test_size"] = CSV_TEST_SIZE
            split_kwargs["random_state"] = CSV_SPLIT_RANDOM_STATE + (
                seed if seed is not None else (random_state if random_state is not None else 42)
            )
        train_idx, test_idx, split_info = create_enhanced_splits(
            standardized_data, split_type=split_type, **split_kwargs
        )
    ...
    def save_results(self):
        ...
        with open(results_file, 'w', encoding='utf-8') as f:
            ...
            def convert_to_serializable(obj):
                if isinstance(obj, (np.integer, np.int64, np.int32, np.int16, np.int8, int)):
                    return int(obj)
...
def main():
    ...
    parser.add_argument(
        "--curves",
        type=str,
        choices=["all", "mean_only", "full", "none"],
        default="mean_only",
        help="训练曲线图：all=每种子+均值图，mean_only=仅均值图，full=同 mean_only（02 专用单模型细图），none=不生成图",
    )
    parser.add_argument(
        "--data-source",
        type=str,
        choices=["json", "csv"],
        default="csv",
        help="json=c13k 权威 JSON + 三种划分；csv=experiments/data/data.csv + train_test（与 02 同源）",
    )
    parser.add_argument(
        "--csv-path",
        type=str,
    ...
    curves_01 = "mean_only" if args.curves == "full" else args.curves
    ...
    if args.data_source == "json":
        print(
            "[提示] 当前运行为 JSON 多划分对照轨。主协议 Problem-Split 建议使用 --data-source csv。",
            flush=True,
        )
    
    try:
        # 初始化增强标准化器（csv 模式仍构造占位，便于复用同一 Trainer 类）
        print("初始化增强数据标准化器...")
